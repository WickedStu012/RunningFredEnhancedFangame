...
public class blValidateReceipt
{
	private const string GAME_ID = "rf_ios";
	...
	public static void ValidateReceipt(string receiptBase64, bool isTest, BlackLordRes cbfn)
	{
		beRes = cbfn;
		string value = string.Format("{0}.{1}.{2}.{3}", "rf_ios", "180", receiptBase64, (!isTest) ? "0" : "1");
		WWWForm wWWForm = new WWWForm();
		wWWForm.AddField("op", "irv");
	...
	public static void Update()
	{
		...
		if (www.isDone)
		{
			...
			if (www.error == null)
			{
				...
				{
					beRes(false, string.Format("[ERR] Error parsing the response of the command ValidateReceipt from server. Response: {0} Error: {1}", text, www.error));
				}
			}
			else
			{
		...
		else
		{
			...
			if (accumTime > 10f)
			{
				Debug.Log("[ERR] ValidateReceipt command timeout");
				waiting = false;
				beRes(false, "Timeout");
			}
		}
