...
public class ActMegaSprint : IAction
{
	...
	private const float maxAccelK = 1.8f;
	...
	public ActMegaSprint(GameObject player)
		: base(player)
	{
		stateName = ActionCode.MEGA_SPRINT;
		smokeTrail = GameObject.FindWithTag("PlayerTrail");
		props = CharHelper.GetProps();
	}

	public void ResetTimerToReturnToRunning(bool triggerSound)
	{
	...
	public override void Update(float dt)
	{
		...
		if (accumTimeSprintLength > props.MegaSprintTime - 0.5f)
		{
			FovAnimator.FovOut();
		}
		if (accumTimeSprintLength > props.MegaSprintTime)
		{
	...
	{
		if (props.MaxMegaSprintSpeed <= sm.Speed && sm.Speed <= props.MaxMegaSprintSpeed * 1.1f)
		{
			incAccelK = 1f;
		}
		else if (sm.Speed > props.MaxMegaSprintSpeed * 1.1f)
		{
			incAccelK = 0.98f;
		}
		else
		{
			incAccelK = props.SuperMegaSprintAccelK;
		}
	}
	...
	private void moveCharacter()
	{
		...
		sm.AccumAccel = Mathf.Clamp(sm.AccumAccel * incAccelK + sm.FloorNormalZ * 0.01f, 0.55f, 1.8f);
